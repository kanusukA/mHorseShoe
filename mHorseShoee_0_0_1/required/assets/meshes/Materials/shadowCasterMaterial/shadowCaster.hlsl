...
VertexVP mainVP(VertexIn p_In,
            uniform float4 ambientLight,          // Ambient light in scene
            uniform float4x4 worldViewProjection  // Model view projection matrix
           )
{
    VertexVP l_Out = (VertexVP) 0;
    
    // Transform vertex position into homogenous screen-space.
    l_Out.position = mul(worldViewProjection, p_In.position);
    
    // Store depth
    l_Out.depth = l_Out.position.z;
    
    // Store ambient color
   // l_Out.color = ambientLight;
